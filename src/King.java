...
public class King implements Token {
	...
	ImageIcon kingIcon;
	int colour;
	HashMap<SquareButton, SquareButton> killerMovemenets; // Stores the location of the movement that can kill and the location of that contains the killed token
	...
	public List<SquareButton> checkPosibleMoves(SquareButton pieceLocation, View gameView) {
		...
		for (int rowUp = row - 1, rowDown = row + 1, columnLeft = column - 1, columnRight = column + 1; (isCheckingUp == true) || (isCheckingDown == true); rowUp--, rowDown++, columnLeft--, columnRight++) {
			
//			Checks the rows up from the position provided
			if (rowUp >= 0 && isCheckingUp == true) {
				
//				Checks the diagonal up left
				if (columnLeft >= 0 && diagonalUpLeftIsBlocked == false && gameView.squares[rowUp][columnLeft] != null) {
					...
					if (gameView.squares[rowUp][columnLeft] != null && gameView.squares[rowUp][columnLeft].isOccupied() == false) {
					...
					} else if (gameView.squares[rowUp][columnLeft].getToken().getColour() == pieceLocation.getToken().getColour()){
						...
						this.killerMovemenets.put(gameView.squares[rowUp - 1][columnLeft - 1], gameView.squares[rowUp][columnLeft]);
...
//				Checks the diagonal up right
				if (columnRight <= 7 && diagonalUpRightIsBlocked == false && gameView.squares[rowUp][columnRight] != null) {
					...
					} else if (gameView.squares[rowUp][columnRight].getToken().getColour() == pieceLocation.getToken().getColour()){
						
						diagonalUpRightIsBlocked = true;
						
					} else if (((rowUp - 1) >= 0) && ((columnRight + 1) <= 7) && gameView.squares[rowUp - 1][columnRight + 1].getToken() == null) {
						
						gameView.squares[rowUp - 1][columnRight + 1].setBackground(Color.red);
						this.killerMovemenets.put(gameView.squares[rowUp - 1][columnRight + 1], gameView.squares[rowUp][columnRight]);
...
//				Checks the diagonal down left
				if (columnLeft >= 0 && diagonalDownLefttIsBlocked == false && gameView.squares[rowDown][columnLeft] != null) {
					...
					} else if (gameView.squares[rowDown][columnLeft].getToken().getColour() == pieceLocation.getToken().getColour()){
						
						diagonalDownLefttIsBlocked = true;
						
					} else if (((rowDown + 1) <= 7) && ((columnLeft - 1) >= 0) && gameView.squares[rowDown + 1][columnLeft - 1].getToken() == null) {
						
						gameView.squares[rowDown + 1][columnLeft - 1].setBackground(Color.red);
						this.killerMovemenets.put(gameView.squares[rowDown + 1][columnLeft - 1], gameView.squares[rowDown][columnLeft]);
...
//				Checks the diagonal down right
				if (columnRight <= 7 && diagonalDownRightIsBlocked == false && gameView.squares[rowDown][columnRight] != null) {
					...
					} else if (gameView.squares[rowDown][columnRight].getToken().getColour() == pieceLocation.getToken().getColour()){
						
						diagonalDownRightIsBlocked = true;
						
					} else if (((rowDown + 1) <= 7) && ((columnRight + 1) <= 7) && gameView.squares[rowDown + 1][columnRight + 1].getToken() == null) {
						
						gameView.squares[rowDown + 1][columnRight + 1].setBackground(Color.red);
						this.killerMovemenets.put(gameView.squares[rowDown + 1][columnRight + 1], gameView.squares[rowDown][columnRight]);
